...
G2_DIR     = Path("data/processed/G2_v2_nohttps")
MODEL_DIR  = Path("results/anomaly")
LSTM_META  = Path("data/processed/G2_v2/lstm_data/char_vocab.json")
OUTPUT_DIR = Path("results/hybrid/anomaly_g3_scores_nohttps")
...
def extract_data_urls(data_csv):
    """Re-extract URLs per email (email_id = row index)."""
    data = pd.read_csv(data_csv)
    rows, email_meta = [], []
    for email_id, row in data.iterrows():
        body = str(row.get("body", "") or "")
        # Selected dataset uses 0=legit, 1=phishing — the OPPOSITE of the project convention
        # (0=phishing, 1=legitimate). Flipped here.
        raw_label = row["label"]
        label = (1 - int(raw_label)) if ("label" in data.columns and not pd.isna(raw_label)) else -1
...
def main():
    ...
    # --- 1. extract URLs ---
    url_df, email_meta = extract_data_urls(CEAS_CSV)
    ...
    feats = feats.drop(columns=["has_https"])
    ...
    iso = joblib.load(MODEL_DIR / "isolation_forest_nohttps" / "isolation_forest.pkl")
    url_df["iso"] = -iso.score_samples(X)


    ff = keras.models.load_model(MODEL_DIR / "autoencoder_ff_nohttps" / "autoencoder_ff.keras")
    ...
    scaler = joblib.load("data/processed/G2_v2_nohttps/standard_scaler.pkl")
